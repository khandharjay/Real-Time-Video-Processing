...
int main(int argc, char *argv[])
{
	...
	//}
        ...
        {
        sprintf(imagename,"%s%d.ppm",image,count);
        //printf("%s",imagename);
    ...
    cycleCnt = cyclesElapsed(stopread, startread);
    microsecs = cycleCnt/clksPerMicro;
    millisecs = microsecs/1000;

    printf("Convolution Read time in cycles=%llu, rate=%llu, about %llu millisecs\n",
    ...
    cycleCnt = cyclesElapsed(stopwrite, startwrite);
    microsecs = cycleCnt/clksPerMicro;
    millisecs = microsecs/1000;

    printf("Convolution Write time in cycles=%llu, rate=%llu, about %llu millisecs\n",
	    cycleCnt, clksPerMicro, millisecs);
    close(fdin);
    close(fdout);
}
